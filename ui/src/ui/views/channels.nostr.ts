...
function truncatePubkey(pubkey: string | null | undefined): string {
  if (!pubkey) {
    return t("views.channels.nostr.na") as unknown as string;
  }
  if (pubkey.length <= 20) {
    return pubkey;
  }
...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          <div>
            <span class="label">${t("views.channels.nostr.running")}</span>
            <span>${account.running ? t("views.channels.nostr.yes") : t("views.channels.nostr.no")}</span>
          </div>
          <div>
            <span class="label">${t("views.channels.nostr.configured")}</span>
            <span>${account.configured ? t("views.channels.nostr.yes") : t("views.channels.nostr.no")}</span>
          </div>
          <div>
            <span class="label">${t("views.channels.nostr.publicKey")}</span>
            <span class="monospace" title="${publicKey ?? ""}">${truncatePubkey(publicKey)}</span>
          </div>
          <div>
            <span class="label">${t("views.channels.nostr.lastInbound")}</span>
            <span>${account.lastInboundAt ? formatAgo(account.lastInboundAt) : t("views.channels.nostr.na")}</span>
          </div>
          ${
            account.lastError
              ? html`
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t("views.channels.nostr.profile.title")}</div>
          ${
            summaryConfigured
              ? html`
                <button
                ...
                >
                  ${t("views.channels.nostr.profile.edit")}
                </button>
              `
              : nothing
          }
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                ...
                ${name ? html`<div><span class="label">${t("views.channels.nostr.profile.name")}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t("views.channels.nostr.profile.displayName")}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t("views.channels.nostr.profile.about")}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
            ...
            : html`
                <div style="color: var(--text-muted); font-size: 13px">
                  ${t("views.channels.nostr.profile.noProfile")}
                </div>
              `
        }
      </div>
  ...
  return html`
    <div class="card">
      <div class="card-title">${t("views.channels.nostr.title")}</div>
      <div class="card-sub">${t("views.channels.nostr.subtitle")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("views.channels.nostr.configured")}</span>
                <span>${summaryConfigured ? t("views.channels.nostr.yes") : t("views.channels.nostr.no")}</span>
              </div>
              <div>
                <span class="label">${t("views.channels.nostr.running")}</span>
                <span>${summaryRunning ? t("views.channels.nostr.yes") : t("views.channels.nostr.no")}</span>
              </div>
              <div>
                <span class="label">${t("views.channels.nostr.publicKey")}</span>
              ...
              <div>
                <span class="label">${t("views.channels.nostr.lastStart")}</span>
                <span>${summaryLastStartAt ? formatAgo(summaryLastStartAt) : t("views.channels.nostr.na")}</span>
              </div>
            </div>
          `
      }
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${t("views.channels.nostr.refresh")}</button>
      </div>
    </div>
  `;
}
