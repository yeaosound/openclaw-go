...
- 运行 {help} 获取帮助`,
  ...
  "wizard.systemd.unavailableSkip":
    "Systemd 用户服务不可用；跳过服务安装。使用您的容器管理器或 `docker compose up -d`。",
  "wizard.systemd.lingerReason":
    "Linux 安装默认使用 systemd 用户服务。没有持久化，systemd 会在注销/空闲时停止用户会话并终止网关。",
  ...
  "wizard.token.webuiStorage":
    "Web 界面将此副本存储在浏览器的 localStorage 中 (openclaw.control.settings.v1)。",
  ...
  "wizard.security.final.desc":
    "在您的计算机上运行代理存在风险 — 加固您的设置：https://docs.openclaw.ai/security",
  ...
  "wizard.completion.outro.dashboard":
    "入门完成。仪表板已在您的浏览器中打开；保留该标签页以控制 OpenClaw。",
  ...
  "wizard.tailscale.warning":
    "在 PATH 或 /Applications 中找不到 Tailscale 二进制文件。确保从 https://tailscale.com/download 安装 Tailscale。您可以继续设置，但 serve/funnel 将在运行时失败。",
